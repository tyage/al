...
var item = function(config){
	...
	this.x = this.y = this.width = this.height = this.link = this.linkDetail = this.jumpRate = this.limit = this.content = this.type = null;
...
item.prototype.changeSpeed = function(){
	...
	switch(this.type){
		...
		case "rand":
			...
			if(this.isBlocked.right && Math.random()*this.jumpRate < 1) this.speed.y = -this.baseSpeed.jy;
			if(this.isBlocked.left && Math.random()*this.jumpRate < 1) this.speed.y = this.baseSpeed.jy;
...
item.prototype.block = function(){
	...
	for(var i=items.length;i>0;i--){
		...
		if(item.isBlockable){
			for(var j=item.isBlockable.length;j>0;j--){
				isBlockable[item.isBlockable[j-1]] = true;
			}
		}
		if(this.x + this.width > item.x && this.x < item.x + item.width){
			if(isBlockable.top && this.old.y + this.height <= item.old.y && this.y + this.height >= item.y){
				if(item.isTreadable) item.beTreated();
				
				if(item.isPushable) item.speed.y += this.speed.y;
				this.speed.y *= -item.bounce.top;
				this.y = item.y - this.height + (item.speed.y < 0 ? item.speed.y : 0);
				this.x += item.speed.x + (item.scroll.top < 0 && !this.isBlocked.left) || (item.scroll.top > 0 && !this.isBlocked.right) ? item.scroll.top : 0;
				this.isBlocked.down = true;
			}
			if(isBlockable.bottom && this.old.y >= item.old.y + item.height && this.y <= item.y + item.height){
				if(this.isTreadable) this.beTreated();
				
				if(item.isPushable) item.speed.y += this.speed.y;
				this.speed.y *= -item.bounce.bottom;
				this.y = item.y + item.height + (item.speed.y > 0 ? item.speed.y : 0);
				this.x += item.speed.x + (item.scroll.bottom < 0 && !this.isBlocked.left) || (item.scroll.bottom > 0 && !this.isBlocked.right) ? item.scroll.bottom : 0;
				this.isBlocked.up = true;
			}
		}
		if(this.y + this.height > item.y && this.y < item.y + item.height){
				if(isBlockable.left && this.old.x >= item.old.x + item.width && this.x <= item.x + item.width){
				if(item.isPushable) item.speed.x += this.speed.x;
				this.speed.x *= -item.bounce.left;
				this.x = item.x + item.width + (item.speed.x > 0 ? item.speed.x : 0);
				this.y += item.speed.y + (item.scroll.left < 0 && !this.isBlocked.up) || (item.scroll.left > 0 && !this.isBlocked.down) ? item.scroll.left : 0;
				this.isBlocked.left = true;
			}
			if(isBlockable.right && this.old.x + this.width <= item.old.x && this.x + this.width >= item.x){
				if(item.isPushable) item.speed.x += this.speed.x;
				this.speed.x *= -item.bounce.right;
				this.x = item.x - this.width + (item.speed.x < 0 ? item.speed.x : 0);
				this.y += item.speed.y + (item.scroll.right < 0 && !this.isBlocked.up) || (item.scroll.right > 0 && !this.isBlocked.down) ? item.scroll.right : 0;
				this.isBlocked.right = true;
			}
		}
		
		if(this.type == "control" && item.href){
			if(this.isTouch(item)) this.reloadLink(item);
		}
	}
...
item.prototype.reloadLink = function(link){
	...
	elems.linkDetail.style.left = (this.x + this.width) + "px";
	elems.linkDetail.style.top = (this.y + this.height) + "px";
};
...
function set(){
	elems = {
		...
		helpContent : document.getElementById("ALhelpContent") || function(){
			var elem = document.createElement("div");
			elem.style.position = "absolute";
			elem.innerHTML = "↑：ジャンプ<br>→：右へ加速<br>←：左へ加速<br>↓：下に加速<br>スペース：リンク先へ飛ぶ";
...
function overwriteObject(old,over){
	for(var key in over){
		if(isObject(over[key])){
			...
			try{
				old[key] = overwriteObject(old[key],over[key]);
			}catch(e){
				//IEだとよく分からんエラーが出るので止める。
			}
		}else{
			old[key] = over[key];
		}
