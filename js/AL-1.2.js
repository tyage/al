...
var chara = function(config){
	...
	this.isMovable = this.isDeleted = this.isControlable = this.isTreadable = this.isDeletable = this.isPushable = false;
	...
	for(var i=charas.length;i>0;i--){
		...
		if(isBlocked.down){
			if(chara.isTreadable){
				chara.isDeleted = true;
				chara.limit = 10;
				img = (this.difx < 0 ? chara.img.tleft : chara.img.tright);
				if(img) chara.elem.setAttribute("src",img);
			}
			
			if(chara.isPushable) chara.speed.y += this.speed.y;
			...
			if(this.isTreadable){
				this.isDeleted = true;
				this.limit = 10;
				img = (this.difx < 0 ? this.img.tleft : this.img.tright);
			...
			if(chara.isPushable) chara.speed.x += this.speed.x;
